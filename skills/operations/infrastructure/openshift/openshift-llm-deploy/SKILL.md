...
Both scripts reach Thanos through a temporary port-forward over the existing
Kubernetes API connection and verify the serving certificate against the
service CA. Never add `--insecure`, never query the monitoring Route, and never
ask for a monitoring token.
...
- that a proven Dynamo model/runtime incompatibility will delete only the DGD
  and its operator-owned components, retain the PVC, and apply standard vLLM;
  infrastructure and endpoint failures will be reported without backend
  replacement.
