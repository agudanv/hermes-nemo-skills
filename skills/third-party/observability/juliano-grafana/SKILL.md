---
name: third-party-juliano-grafana
...
- **Variable interpolation**: `$__interval` and `$__name` are interpreted differently in legacy vs newer query backends; same dashboard JSON behaves differently.
- **Provisioning datasources via YAML** under `/etc/grafana/provisioning`: edits via UI are silently reverted on file re-read; pick one source of truth.
- **API tokens are per-org** — multi-org Grafana setups need separate tokens; a global "admin" token doesn't span orgs.
